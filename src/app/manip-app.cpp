...
public:
	...
	virtual double evaluate(const VectorXd &u) const {
		...
		double O = get_O(x);
        if (QUESTION_12) {
            // ridge regularizer = O(x(u)) + lambda * ((u(theta_L) -90)^2 +(u(theta_R) -90)^2)
			double lambda = 0.1; // scaling factor...arbitrary
			double ridge = lambda *( u(u.size()-1) -3.14159*0.5)* (u(u.size()-1)-3.14159*0.5) + lambda * (u(u.size()-2)-3.14159*0.5) * (u(u.size()-2)-3.14159*0.5); 
			O += ridge;
        }
	...
	VectorXd get_DODu(const VectorXd &u) const {
		...
		VectorXd DODu; DODu.setZero(u.size()); {
			...
			if (step == FD_O_of_u) {
				...
				// NOTES:
				// F = -dE/dx
				// DF/Du = dF/du + dF/dx * dx/du = 0 
				// dx/du = inv(dF/dx) * (DF/du-dF/du)
				// get_H(u, x) == hessian d2E/dx2 == dF/dx
				// solve_AX_EQUALS_B(A,B) solves a matrix system of equations

				SparseMatrixd dFdx = -get_H(x,u);
				...
				// cout<<" Got dFdx: "<<dFdx<<endl;
				// // TODO: SparseMatrixd dFdx = ...
				SparseMatrixd dFdu = get_dFdu(u, x); // TODO: Finish implementing get_dOdx(...)
				// // TODO: MatrixXd dxdu = ...
				// // --
				// cout<<" Got dFdu: "<<dFdu<<endl;
				MatrixXd dxdu = solve_AX_EQUALS_B(dFdx,-dFdu);
				// cout<<" Got dxdu: "<<dxdu<<endl;
				// // TODO: DODu += ...
				DODu += dxdu.transpose()*dOdx;
				// cout<<" Got DODu: "<<DODu<<endl;
			}
	...
	VectorXd get_dOdx(const VectorXd &x) const {
		...
		// Vector2d x_i = seg2(x, i) or Vector2d x_i = x.segment<2>(2 * i);
		...
		VectorXd dOdx = VectorXd::Zero(sim->x.size());
		for (const int &i : I) {
		...
		SparseMatrixd ret; ret.resize(x.size(), u.size()); {
			for (const auto &pin : sim->pins) { 
				...
				MatrixXd dp_primedu; dp_primedu.setZero(2, u.size()); {
					...
					// p = [x' y'].T
					// 			
					// dp'/du = | dx'/dx_L  dx'/dx_R  dx'/dtheta_L  dx'/dtheta_R | 
					//			| dy'/dx_L	dy'/dx_R  dy'/dtheta_L  dy'/dtheta_R |
					...
					// Notes:
					// f = [cos sin ;sin cos] * [f(0) f(1)] = R *f
					// f' = [-sin cos ;cos -sin] * [f(0) f(1)] = R' *f
					// cout<< "dp_primedu before: " << dp_primedu << endl;

					Matrix2d R_prime ;
					R_prime << -sin(theta), cos(theta), cos(theta), -sin(theta);
					Vector2d f_prime = R_prime * f;
					dp_primedu.col(theta_dataIndex) = f_prime;
					// cout<< "dp_primedu after: " << dp_primedu << endl;
				} 
